...
func TestJWTAuth_SuccessfulAuthentication(t *testing.T) {
	...
	// Create test handler
	handler := JWTAuth(env)(createTestHandler(t, userID, deviceID))
...
func TestJWTAuth_MissingAuthorizationHeader(t *testing.T) {
	...
	handler := JWTAuth(env)(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
...
func TestJWTAuth_MismatchedKeys(t *testing.T) {
	...
	handler := JWTAuth(verifyEnv)(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		t.Error("handler should not be called")
	}))
